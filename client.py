...
import socket
import threading
from Pipline import EncodePipline
...
def DealOut(sock):
    ...
    while True:
        outString = input(nick+": ")  # 输入
        outString = nick + ':' + outString  # 拼接cd
        sock.send(EncodePipline(outString).encode('utf-8'))  # 发送
...
sock.connect((ip, 5000))  # 发起请求，接收的是一个元组
